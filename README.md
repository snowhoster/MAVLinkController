...
| 方向盤電位計 | WH148 B10K 10kΩ 線性旋轉電位計 ＋ Φ15mm×H13mm 防滑旋鈕帽（3.3V 供電，Wiper 直入 A0） | 1 |
...
### 類比輸入（方向盤電位計 ＆ APEM SN 雙 T-Bar 霍爾推桿）

系統包含三路主要類比輸入，因感測器電氣特性不同採用分立供電架構：

#### 1. 方向盤電位計（WH148 B10K — 直供 3.3V）
```
3.3V ──┬── [電位計 Pin 1]
       │
       ├── [電位計 Pin 2] ──┬──→ Arduino A0 (方向盤舵角, 0~3.3V)
       │                   └──┤├── GND (100nF 陶瓷濾波電容)
       └── [電位計 Pin 3] ─────── GND
```
